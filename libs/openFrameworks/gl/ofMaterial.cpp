...
// VWCUSTOM - ADDED FUNCTIONS BEGIN
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

bool ofMaterial::define_basic_shader(basic_shader_function_type_enum basic_shader_function, ofGLProgrammableRenderer *renderer, ofShader *basic_shader) {
	if (basic_shader == NULL || renderer == NULL) {
		// error message
		return false;
	}
	...
	// VWCUSTOM SUBSTITUTE BEGIN SSSSSSSSSSSSSSSSSSSSSSS
	size_t numLights = ofLightsData().size();

	//size_t numLights = vw_lights_data().size();

	// SUBSTITUTE END SSSSSSSSSSSSSSSSSSSSSSS
	// only support for a single cube map at a time
	size_t numCubeMaps = ofCubeMapsData().size() > 0 ? 1 : 0;
	const std::string shaderId = getShaderStringId();
	...
	std::string extraVertString = definesString;
	//		if( hasTexture(VW_MATERIAL_TEXTURE_DISPLACEMENT) ) {
	//			extraVertString += "\nuniform SAMPLER "+getUniformName(VW_MATERIAL_TEXTURE_DISPLACEMENT)+";\n";
	//		}
	extraVertString += customUniforms;
	ofLogVerbose("ofMaterial") << " extraVertString------------------- ";
	ofLogVerbose() << extraVertString;
	...
	//ofShader *basic_shader;
	//basic_shader = new ofShader;

	//if (basic_shader == NULL) return NULL;

	//shaders[&renderer].reset(new Shaders);
	//shaders[&renderer]->numLights = numLights;
	//shaders[&renderer]->numCubeMaps = numCubeMaps;
	//shaders[&renderer]->shaderId = shaderId;
	switch (basic_shader_function) {
			case basic_shader_function_type_enum::no_texture: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertex2DHeader, numLights, false, false, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragment2DHeader, customUniforms, data, numLights, false, false, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}

			case basic_shader_function_type_enum::texture_2D: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertex2DHeader, numLights, true, false, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragment2DHeader, customUniforms, data, numLights, true, false, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}
		...
		#ifndef TARGET_OPENGLES
			case basic_shader_function_type_enum::texture_rect: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertexRectHeader, numLights, true, false, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragmentRectHeader, customUniforms, data, numLights, true, false, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}
		#endif
			case basic_shader_function_type_enum::color: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertex2DHeader, numLights, false, true, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragment2DHeader, customUniforms, data, numLights, false, true, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}

			case basic_shader_function_type_enum::texture_2D_color: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertex2DHeader, numLights, true, true, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragment2DHeader, customUniforms, data, numLights, true, true, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}
		...
		#ifndef TARGET_OPENGLES
			case basic_shader_function_type_enum::texture_rect_color: {
				basic_shader->setupShaderFromSource(GL_VERTEX_SHADER, vertexSource(isPBR(), vertexRectHeader, numLights, true, true, extraVertString, data));
				basic_shader->setupShaderFromSource(GL_FRAGMENT_SHADER, fragmentSource(isPBR(), fragmentRectHeader, customUniforms, data, numLights, true, true, definesString));
				//basic_shader->bindDefaults();
				//basic_shader->linkProgram();
				break;
			}
	...
	if (!basic_shader->bindDefaults()) {
		std::cout << "!basic_shader->bindDefaults() !!!!!!!!!!!!!!\n";
		return false;
	}

	if (!basic_shader->linkProgram()) {
		std::cout << "!basic_shader->linkProgram()\n !!!!!!!!!!!!!!!!!";
		return false;
	}
...
std::string ofMaterial::basic_shader_source_code(shader_type_enum shader_type,basic_shader_function_type_enum basic_shader_function, ofGLProgrammableRenderer* renderer, bool set_as_pbr_shader = false) {
	if (renderer->getGLVersionMajor() < 4) {
		// error message
		return false;
	}
	...
	if (shader_type == shader_type_enum::geometry) {
		std::string source_code = "";
	...
	switch (basic_shader_function) {
		case basic_shader_function_type_enum::no_texture: {
			if(shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertex2DHeader, numLights, false, false, extraVertString, data);
			if(shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragment2DHeader, customUniforms, data, numLights, false, false, definesString);
			break;
		}

		case basic_shader_function_type_enum::texture_2D: {
			if (shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertex2DHeader, numLights, true, false, extraVertString, data);
			if (shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragment2DHeader, customUniforms, data, numLights, true, false, definesString);
			break;
		}

	#ifndef TARGET_OPENGLES
		case basic_shader_function_type_enum::texture_rect: {
			if (shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertexRectHeader, numLights, true, false, extraVertString, data);
			if (shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragmentRectHeader, customUniforms, data, numLights, true, false, definesString);
			break;
		}
	#endif
		case basic_shader_function_type_enum::color: {
			if (shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertex2DHeader, numLights, false, true, extraVertString, data);
			if (shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragment2DHeader, customUniforms, data, numLights, false, true, definesString);
			break;
		}

		case basic_shader_function_type_enum::texture_2D_color: {
			if(shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertex2DHeader, numLights, true, true, extraVertString, data);
			if (shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragment2DHeader, customUniforms, data, numLights, true, true, definesString);
			break;
		}

	#ifndef TARGET_OPENGLES
		case basic_shader_function_type_enum::texture_rect_color: {
			if (shader_type == shader_type_enum::vertex) return vertexSource(set_as_pbr_shader, vertexRectHeader, numLights, true, true, extraVertString, data);
			if (shader_type == shader_type_enum::fragment) return fragmentSource(set_as_pbr_shader, fragmentRectHeader, customUniforms, data, numLights, true, true, definesString);
			break;
		}
	#endif
	}
...
// VWCUSTOM - ADDED FUNCTIONS BEGIN
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
std::string ofMaterial::shader_header(std::string header, int maxLights, bool hasTexture, bool hasColor) {
...
std::string ofMaterial::vertex_source(bool bPBR, std::string defaultHeader, int maxLights, bool hasTexture, bool hasColor, std::string addShaderSrc, const ofMaterialSettings& adata) {
	...
	if (bPBR) {
		...
		if (mainVertex.empty()) {
			mainVertex = shader_pbr_main_vert;// From #include "shaders/pbr.vert"
		}
		ofStringReplace(source, "%mainVertex%", mainVertex);
	}

	return shader_header(defaultHeader, maxLights, hasTexture, hasColor) + source;
}
// const material_settings& adata
std::string ofMaterial::fragment_source(bool bPBR, std::string defaultHeader, std::string customUniforms, const ofMaterialSettings& adata, int maxLights, bool hasTexture, bool hasColor, std::string definesString) {
	...
	if (bPBR) {
		...
		if (mainFrag.empty()) {
			if (bPBR) {
				mainFrag = shader_pbr_main_frag;// From #include "shaders/pbr.frag"
			}
		}
	...
	source = shader_header(defaultHeader, maxLights, hasTexture, hasColor) + definesString + source;
	return source;
}
// VWCUSTOM - ADDED FUNCTIONS END
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
