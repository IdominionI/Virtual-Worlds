...
bool ofShader::load(const of::filesystem::path& shaderName) {
	...
	compile_log = "";
	shader_compile_successful = true;
	// VW_CUSTOM - ADDED END ++++++++++++++++++
...
bool ofShader::load(const of::filesystem::path& vertName, const of::filesystem::path& fragName, const of::filesystem::path& geomName) {
	...
	if(vertName.empty() == false) if(!setupShaderFromFile(GL_VERTEX_SHADER, vertName)) shader_compile_successful = false;
	if(fragName.empty() == false) if(!setupShaderFromFile(GL_FRAGMENT_SHADER, fragName)) shader_compile_successful = false;
#ifndef TARGET_OPENGLES
	if(geomName.empty() == false) if(!setupShaderFromFile(GL_GEOMETRY_SHADER_EXT, geomName)) shader_compile_successful = false;
#endif
	...
	if(shader_compile_successful)
		shader_compile_successful = linkProgram();
...
bool ofShader::setupShaderFromFile(GLenum type, const of::filesystem::path & filename) {
	...
	if(buffer.size()) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader setupShaderFromFile(): couldn't load " + nameForType(type) + " shader from \"" + absoluteFilePath + "\"" + "\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return false;
	}
}
...
bool ofShader::setupShaderFromSource(ofShader::Source && source){
	...
	if(shaderId == 0) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader setupShaderFromSource(): failed creating " + nameForType(source.type) + " shader\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return false;
	} else {
	...
	if (err != GL_NO_ERROR){
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader : setupShaderFromSource(): OpenGL generated error " + std::to_string(err) + " trying to get the compile status for a " + nameForType(shader.source.type) + " shader, does your video card support this?\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return false;
	}
	...
	if(status == GL_TRUE){
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader : setupShaderFromSource():" + nameForType(shader.source.type) + " shader failed to compile\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
...
string ofShader::parseForIncludes( const string& source, vector<string>& included, int level, const of::filesystem::path& sourceDirectoryPath) {
	...
	if ( level > 32 ) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		//compile_log += "[ERROR] ofShader : glsl header inclusion depth limit reached, might be caused by cyclic header inclusion\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return "";
	}
	...
	while( std::getline( input, line ) ) {
		...
		if ( !buffer.size() ) {
			...
			// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
			//compile_log += "[ERROR] ofShader : Could not open glsl include file " + include + "\n";
			// VW_CUSTOM - ADDED END ++++++++++++++++++
			continue;
		}
...
string ofShader::getShaderSource(GLenum type)  const{
	...
	if ( source != shaders.end()) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		//compile_log += "[ERROR] ofShader : No shader source for shader of type:  " +nameForType(type) + "\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return "";
	}
}
...
bool ofShader::checkProgramLinkStatus() {
	...
	if (err != GL_NO_ERROR){
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader :checkProgramLinkStatus(): OpenGL generated error " + std::to_string(err) + " trying to get the program link status, does your video card support shader programs?\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return false;
	}
	if(status == GL_TRUE){
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader : checkProgramLinkStatus(): program failed to link\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		checkProgramInfoLog();
		return false;
	}
...
void ofShader::checkShaderInfoLog(GLuint shader, GLenum type, ofLogLevel logLevel) {
	...
	if (infoLength > 1) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "ofShader: shader reports: \n" + nameForType(type) +  infoBuffer.getText().c_str()+"\n";
		//compile_log += shaders[type].source.expandedSource;
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		...
		if (shaders.find(type) != shaders.end()) {
			...
			if (std::regex_search(infoString, matches, intel) || std::regex_search(infoString, matches, nvidia_ati)){
				...
				// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
//std::cout << "IN checkProgramInfoLog() 1111\n";
				compile_log += shaders[type].source.expandedSource;
				compile_log += "BIG ERRORRRRR2222\n";
//std::cout << "IN checkProgramInfoLog() 2222\n";
				shader_compile_successful = false;
				// VW_CUSTOM - ADDED END ++++++++++++++++++
			}
		}
...
void ofShader::checkProgramInfoLog() {
	...
	if (infoLength > 1) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += msg.c_str() + infoBuffer.getText();
		shader_compile_successful = false;
		// VW_CUSTOM - ADDED END ++++++++++++++++++
...
bool ofShader::linkProgram() {
	if(shaders.empty()) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader : linkProgram(): trying to link GLSL program, but no shaders created yet\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
	} else {
		checkAndCreateProgram();
...
bool ofShader::bindDefaults(){
	if(shaders.empty()) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		compile_log += "[ERROR] ofShader : bindDefaults(): trying to link GLSL program, but no shaders created yet\n"; 
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return false;
	} else {
...
GLint ofShader::getUniformBlockBinding( const string & name ) const{
	...
	if(GLEW_ARB_uniform_buffer_object) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		//compile_log += "[ERROR : ofShader::getUniformBlockBinding\n Sorry, it looks like you can't run 'ARB_uniform_buffer_object'\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		return -1;
	}
}
...
void ofShader::printActiveUniformBlocks()  const{
	if(GLEW_ARB_uniform_buffer_object) {
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		//compile_log += "[NOTICE] : ofShader " + std::to_string(numUniformBlocks) + " uniform blocks\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
		...
		// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
		//compile_log += "[ERROR : ofShader::printActiveUniformBlocks\n Sorry, it looks like you can't run 'ARB_uniform_buffer_object'\n";
		// VW_CUSTOM - ADDED END ++++++++++++++++++
	}
}
...
void ofShader::bindUniformBlock(GLuint binding, const string & name) const{
	if(bLoaded){
		if(GLEW_ARB_uniform_buffer_object) {
			...
			// VW_CUSTOM - ADDED BEGIN ++++++++++++++++++
			//compile_log += "[ERROR : ofShader::bindUniformBlock\n Sorry, it looks like you can't run 'ARB_uniform_buffer_object'\n";
			// VW_CUSTOM - ADDED END ++++++++++++++++++
		}
	}
}
