...
// VWCUSTOM - ADDED INCLUDES BEGIN
// *******************

#include "vw_shader_definitions.h"

// *******************
// VWCUSTOM - ADDED INCLUDES end
...
public:
	...
	//ofShader *define_basic_shader(basic_shader_function_type_enum basic_shader_function,ofAppBaseWindow *window) {
	
	// Cannot use this function as get compilor error overloaded function not found in "material_shader_class" despite
	// this function call being consitant in the .cpp file. It is found that the type ofAppBaseWindow* window is the cause
	// of the compilation error and leads to the conclusion that openframe works is totaly unreliable to be used as a basis
	// to be used with modifications or additions for application development. !!!!!!!!
	//bool define_basic_shader(basic_shader_function_type_enum basic_shader_function, ofAppBaseWindow* window, ofShader* basic_shader);
	
	bool define_basic_shader(basic_shader_function_type_enum basic_shader_function, ofGLProgrammableRenderer *renderer, ofShader* basic_shader);

	std::string basic_shader_source_code(shader_type_enum shader_type, basic_shader_function_type_enum basic_shader_function, ofGLProgrammableRenderer* renderer, bool set_as_pbr_shader);

	std::string shader_header(std::string header, int maxLights, bool hasTexture, bool hasColor);
	std::string vertex_source(bool bPBR, std::string defaultHeader, int maxLights, bool hasTexture, bool hasColor, std::string addShaderSrc, const ofMaterialSettings& adata);
	std::string fragment_source(bool bPBR, std::string defaultHeader, std::string customUniforms, const ofMaterialSettings& adata, int maxLights, bool hasTexture, bool hasColor, std::string definesString);
	
	ofShader *get_shader(int textureTarget, bool geometryHasColor, ofGLProgrammableRenderer& renderer) {
		initShaders(renderer);

		if (bHasCustomShader && customShader) {
			return customShader.get();
		}
		...
		if (textureTarget == -1 && hasTexture(OF_MATERIAL_TEXTURE_DIFFUSE)) {
