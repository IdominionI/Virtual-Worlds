...
//#include <iostream> // For debugging only :: comment out when no longer needed

#include "imnodes.h"
#include "imnodes_internal.h"

//#define IMGUI_DEFINE_MATH_OPERATORS // This no longer valid need to define ones own vector math operators
                                      // Need to define ones own vector math operators. Defined in 
                                      // Kernal/node_funtions.h
#include "Kernal/node_functions.h"    // Because IMGUI_DEFINE_MATH_OPERATORS no longer valid 
...
void DrawGrid(ImNodesEditorContext& editor, const ImVec2& canvas_size)
{
    ...
    for (float x = fmodf(offset.x, GImNodes->Style.GridSpacing); x < canvas_size.x/ zoom_scale;x += GImNodes->Style.GridSpacing)
    {
        if (fmodf(x - offset.x, GImNodes->Style.GridSpacing * 5.0 ) == 0)
            line_thickness = 4.0 * zoom_scale;
        else
            line_thickness = 1.0 * zoom_scale;

        GImNodes->CanvasDrawList->AddLine(
            EditorSpaceToScreenSpace(ImVec2(x * zoom_scale, 0.0f)),
            EditorSpaceToScreenSpace(ImVec2(x * zoom_scale, canvas_size.y)),
            GImNodes->Style.Colors[ImNodesCol_GridLine], line_thickness);
    }

     for (float y = fmodf(offset.y, GImNodes->Style.GridSpacing); y < canvas_size.y / zoom_scale;y += GImNodes->Style.GridSpacing)
    {
        if (fmodf(y  -offset.y, GImNodes->Style.GridSpacing *5.0 ) == 0)
            line_thickness = 4.0 * zoom_scale;
        else
            line_thickness = 1.0 * zoom_scale;

        GImNodes->CanvasDrawList->AddLine(
            EditorSpaceToScreenSpace(ImVec2(0.0f, y * zoom_scale)),
            EditorSpaceToScreenSpace(ImVec2(canvas_size.x, y * zoom_scale)),
            GImNodes->Style.Colors[ImNodesCol_GridLine], line_thickness);
    }
}
