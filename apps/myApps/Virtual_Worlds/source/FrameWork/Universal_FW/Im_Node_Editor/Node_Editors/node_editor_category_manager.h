...
public:
	...
	node_id_type editor_node_category_id;
	...
	bool add_editor_node(editor_node_basis* editor_node) {
	...
	bool delete_editor_node(node_type_struct editor_node_definition) {
	...
	bool perform_editor_node_creation(node_type_struct editor_node_definition, graph_class *graph,  ImVec2 click_pos) {
		int index = find_editor_node_creation_function(editor_node_definition);
std::cout << "nodes_of_category_class :perform_editor_node_creation 000 " << editor_category_nodes.size() << " : " << index << std::endl;
		if (index < 0) {
			//if (log_panel != NULL) {}//error_message;
			return false;
		}
std::cout << "nodes_of_category_class :perform_editor_node_creation 001" << editor_category_nodes.size() << " : " << index << std::endl;
		editor_category_nodes[index]->create_editor_node(editor_node_definition, graph,  click_pos);
		return true;
	}
	...
	int find_editor_node_creation_function(node_type_struct editor_node_definition) {
		for (int i = 0; i < editor_category_nodes.size(); i++) {
			...
			//	)
			if (editor_category_nodes[i]->editor_node_definition.node_context == editor_node_definition.node_context)
				return i;
		}
...
public:
	...
	int add_editor_node_category(node_id_type editor_nodes_category_id) {
	//int add_editor_node_category(node_id_type editor_nodes_category_id, log_panel_class* log_panel) {
	...
	//bool add_editor_node(editor_node_basis* editor_node, scene_manager_class* scene_manager, log_panel_class* log_panel) {
	bool add_editor_node(editor_node_basis* editor_node) {
		int index = find_editor_node_creation_function_category(editor_node->editor_node_definition.node_type);
		if (index < 0) {
			//index = add_editor_node_category(editor_node->editor_node_definition.category, log_panel);
			index = add_editor_node_category(editor_node->editor_node_definition.node_type);
			if (index < 0) {
				//if (log_panel != NULL) {
					//error message
				//}
		...
		return nodes_of_category[index]->add_editor_node(editor_node);
	}

	bool delete_editor_node_category(node_id_type editor_category_type_id) {
	//bool delete_editor_node_category(node_id_type editor_category_type_id, log_panel_class* log_panel) {
	...
	bool perform_editor_node_creation(node_type_struct editor_node_definition, graph_class *graph, ImVec2 click_pos) {
	//bool perform_editor_node_creation(node_type_struct editor_node_definition, graph_class *graph, scene_manager_class* scene_manager, log_panel_class* log_panel, ImVec2 click_pos) {
std::cout << "perform_editor_node_creation 001 : " << editor_node_definition.node_type <<std::endl;
		int index = find_editor_node_creation_function_category(editor_node_definition.node_type);
std::cout << "node_editor_category_manager_class :perform_editor_node_creation : " << nodes_of_category.size() << " : " << index << std::endl;
		if (index < 0) {
		...
		return nodes_of_category[index]->perform_editor_node_creation(editor_node_definition, graph, click_pos);
	}

	int find_editor_node_creation_function_category(node_id_type editor_category_type_id) {
std::cout << "find_editor_node_creation_function_category 000A :: " << nodes_of_category.size() << std::endl;
		for (int i = 0; i < nodes_of_category.size(); i++) {
			if (nodes_of_category[i]->editor_node_category_id == editor_category_type_id)
				return i;
		}
std::cout << "find_editor_node_creation_function_category 001 :: " << std::endl;
		return -1;
	}
};
