...
enum class vw_camera_settings_enum{is_orthoraphic,FOV,near_clip,far_clip,lens_offset,force_aspect_ratio,aspect_ratio,vFlip};

struct vw_camera_settings_struct_type {
	bool      isOrtho          = false;
	float     fov              = 60.0f;
	float     nearClip         = 0.0f;
	float     farClip          = 0.0f;
	...
	float     aspectRatio      = 4.0/3.0; // only used when forceAspect=true, = w / h
	bool      vFlip            = false;
...
//class vw_camera_base_class : public vw_node_class {
class vw_camera_base_class : public ofNode {
public:
	/// \name Constructor and Destructor
	/// \{

	/// \brief Construct a default camera.
	vw_camera_base_class();

	/// \brief Destroy the camera.
	virtual ~vw_camera_base_class();

	bool display_active  = false;
	bool display_overlay = false;

	vw_camera_settings_struct_type camera_settings;

	void define_camera_settings(vw_camera_settings_struct_type _camera_settings) {
		camera_settings = _camera_settings;
		setFov(_camera_settings.fov);
		setNearClip(_camera_settings.nearClip);
		setFarClip(_camera_settings.farClip);
		setLensOffset(_camera_settings.lensOffset);
		setForceAspectRatio(_camera_settings.forceAspectRatio);
		setAspectRatio(_camera_settings.aspectRatio);
		setVFlip(_camera_settings.vFlip);
	}

	vw_camera_settings_struct_type get_camera_settings() {
