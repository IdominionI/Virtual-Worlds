...
void voxel_hcp_automata_widget_class::display() {
	if (!voxel_hcp_automata_byte_rules) {
		if (log_panel != NULL) log_panel->application_log.AddLog("ERROR:: Cannot define/execute an automata voxel hcp rule. No automata data defined .\n");
		return;
	}
	...
	hcp_automata_rules_widget.rules_manager(*voxel_hcp_automata_byte_rules);
...
void voxel_hcp_automata_widget_class::clear_automata_rules() {
	voxel_hcp_automata_byte_rules->clear();
}
...
bool voxel_hcp_automata_widget_class::perform_automata_step(int step) {
	...
	if (!voxel_hcp_object_to_execute){
		if (log_panel != NULL) log_panel->application_log.AddLog("ERROR", "perform_automata_step():: No voxel hcp object specified to execute");
		return false;
	}
