...
public:
	...
	bool export_voxel_center_points_ply(vw_scene_objects_category_class &voxel_hcp_entities_to_export,int export_selection, id_type entity_id, bool export_colors = false) {

		if (voxel_hcp_entities_to_export.category_objects.size() < 1) {
	...
	bool export_voxel_center_points_ply_selected(std::string directory_pathname, vw_scene_objects_category_class &voxel_hcp_entities_to_export, id_type entity_id, bool export_colors) {
		...
		vw_object_base_class *object_to_export = voxel_hcp_entities_to_export.get_entity_object(entity_id);
		voxel_hcp_object_class *voxel_hcp_object_to_export = dynamic_cast<voxel_hcp_object_class* >(object_to_export);
		...
		//filename_to_write = directory_pathname + "/cp_" + voxel_hcp_object_to_export->name + ".ply";
		filename_to_write = directory_pathname + "/cp_" + voxel_hcp_object_to_export->name + ".ply";
...
/*
	...
	bool export_voxel_center_points_ply_active(std::string directory_pathname, std::vector<vw_object_base_class*> &voxel_hcp_entities_to_export, bool export_colors) {
		for (index_type i = 0; i < voxel_hcp_entities_to_export.size(); i++) {
			...
			if (voxel_hcp_object_to_export->active_object) {
				if (voxel_hcp_object_to_export->voxel_object_data.voxel_matrix_data.size() > 0) {
					filename_to_write = directory_pathname + "/cp_" + std::to_string(i) + "_" + voxel_hcp_object_to_export->name + ".ply";
	...
	bool export_voxel_center_points_ply_all(std::string directory_pathname, std::vector<vw_object_base_class*> &voxel_hcp_entities_to_export, bool export_colors) {
		for (index_type i = 0; i < voxel_hcp_entities_to_export.size(); i++) {
			voxel_hcp_object_class *voxel_hcp_object_to_export = voxel_hcp_entities_to_export[i];
...
*/
	...
	bool export_voxel_point_surface_data_ply(vw_scene_objects_category_class *voxel_hcp_entities_to_export, int export_selection, id_type entity_id, bool export_colors = false, bool export_faces = false) {
		if (voxel_hcp_entities_to_export->category_objects.size() < 1) {
		if (log_panel != NULL) log_panel->application_log.AddLog("ERROR : Export voxel surface geometry :: Cannot export voxel point surface to file:: No entity data in scene to export\n");
			vwDialogs::message_box("ERROR : Export voxel surface geometry", "Cannot export voxel point surface to file::No entity data in scene to export");
			return false;
	...
	bool export_voxel_point_surface_ply_selected(std::string directory_pathname, vw_scene_objects_category_class *voxel_hcp_entities_to_export, id_type entity_id, bool export_colors, bool export_faces) {
		if (entity_id < 0) {
...
/*
	...
	bool export_voxel_point_surface_ply_active(std::string directory_pathname, std::vector<vw_object_base_class*> & voxel_hcp_entities_to_export, bool export_colors, bool export_faces) {
		...
		for (index_type i = 0; i < voxel_hcp_entities_to_export.size(); i++) {
			...
			if (voxel_entity_to_export->active_object){
				if (voxel_entity_to_export->voxel_object_data.voxel_matrix_data.size() > 0) {
					filename_to_write = directory_pathname + "/sp_" + std::to_string(i) + "_" + voxel_entity_to_export->name + ".ply";

					if (!export_point_voxel_surface_ply_data(voxel_entity_to_export, filename_to_write, export_colors, export_faces))
						exported_all = false;

				}else{
					if (log_panel != NULL) log_panel->application_log.AddLog("ERROR : Voxel Volume to Voxel Surface :: Could not generate voxel surface face data for \n %s \nNo voxel vertex data for  voxel enitity\n", voxel_entity_to_export->name.c_str());
					exported_all = false;
				}
			}
	...
	bool export_voxel_point_surface_ply_all(std::string directory_pathname, std::vector<vw_object_base_class*>& voxel_hcp_entities_to_export, bool export_colors, bool export_faces) {
		bool exported_all = true;
...
*/
	...
	bool export_voxel_surface_faces_data_ply(vw_scene_objects_category_class *voxel_hcp_entities_to_export, int export_selection, id_type entity_id) {
		if (voxel_hcp_entities_to_export->category_objects.size() < 1) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR : Export voxel surface geometry :: Cannot export voxel point surface to file:: No entity data in scene to export\n");
			vwDialogs::message_box("ERROR : Export voxel surface geometry", "Cannot export voxel point surface to file::No entity data in scene to export");
			return false;
	...
	bool export_voxel_surface_faces_ply_selected(std::string directory_pathname, vw_scene_objects_category_class *voxel_hcp_entities_to_export, id_type entity_id) {
		if (entity_id < 0) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR : Export voxel surface geometry :: Cannot export voxel point surface to file:: No entity selected to export\n");
			vwDialogs::message_box("Export voxel surface geometry", "Cannot export voxel point surface to file::No entity selected to export");
			return false;
		...
		filename_to_write = directory_pathname + "/sf_" + voxel_hcp_object_to_export->name + ".ply";
...
/*
	bool export_voxel_surface_faces_ply_active(std::string directory_pathname, voxel_hcp_scene_objects_class& voxel_hcp_entities_to_export) {
		...
		for (index_type i = 0; i < voxel_hcp_entities_to_export.size(); i++) {
			...
			if (voxel_entity_to_export->active_object) {
				if (voxel_entity_to_export->voxel_object_data.voxel_matrix_data.size() > 0) {
					filename_to_write = directory_pathname + "/sf_" + std::to_string(i) + "_" + voxel_entity_to_export->name + ".ply";
	...
	bool export_voxel_surface_faces_ply_all(std::string directory_pathname, std::vector<vw_object_base_class*> &voxel_hcp_entities_to_export) {
		bool exported_all = true;
...
*/
	...
	bool export_voxel_surface_faces_ply(voxel_hcp_scene_objects_class &voxel_hcp_entities_to_export) {
		...
		for (index_type i = 0; i < voxel_hcp_entities_to_export.category_objects.size(); i++) {
			vw_object_base_class *entity_to_export = voxel_hcp_entities_to_export.category_objects[i];
			voxel_hcp_object_class *voxel_entity_to_export = dynamic_cast<voxel_hcp_object_class*>(entity_to_export);
...
private:
		...
		for (voxel_index = first_voxel_export_index; voxel_index < last_voxel_export_index; voxel_index++) {
			...
			if (voxel_value != DEFAULT_INACTIVE_VALUE) {
				...
				if (export_colors) {
					...
					stream << red_value << " " << green_value << " " << blue_value << " " << voxel_value << std::endl;
				} else {
					stream << voxel_cartesian_coordinate.x << " " << voxel_cartesian_coordinate.y << " " << voxel_cartesian_coordinate.z << std::endl;
				}
			}
		}
	...
	void create_file_ply_header(size_t number_voxels_to_export, bool export_colors) {
		...
		stream << "comment exported from virtual worlds " << std::endl;
		stream << "comment Voxel surface data" << std::endl;
		stream << "element vertex " << number_voxels_to_export << std::endl;
	...
	void write_voxel_surface2_vertex_data_to_obj_file(std::vector<surface_vertices_struct_type>& surface_vertices, size_t number_vertices_to_export, voxel_object_data_class& voxel_object_data) {
		...
		for (index = 0; index < number_vertices_to_export; index++) {
			...
			stream << "v " << voxel_cartesian_coordinate.x << " " << voxel_cartesian_coordinate.y << " " << voxel_cartesian_coordinate.z << std::endl;
		}
	}
	...
	void write_voxel_surface2_vertex_normal_data_to_obj_file(std::vector<surface_vertices_normal_struct_type>& surface_vertices_normal, size_t number_vertex_normals_to_export, voxel_object_data_class& voxel_object_data) {
		...
		for (index = 0; index < number_vertex_normals_to_export; index++) {
			...
			stream << "vn " << surface_vertices_normal[index].voxel_normal.x << " " << surface_vertices_normal[index].voxel_normal.y << " " << surface_vertices_normal[index].voxel_normal.z << std::endl;
		}
	}
	...
	void create_voxel_point_surface_ply_header(size_t number_vertices_to_export, size_t number_faces_to_export, bool export_colors, bool export_faces) {
		...
		stream << "comment exported from virtual worlds " << std::endl;
		stream << "comment Voxel surface data" << std::endl;
		stream << "element vertex " << number_vertices_to_export << std::endl;
		stream << "property float x" << std::endl;
		stream << "property float y" << std::endl;
		stream << "property float z" << std::endl;
		...
		if (export_faces) {
			stream << "element face " << number_faces_to_export << std::endl;
	...
	void write_voxel_point_surface_ply_vertex_data_to_file(std::vector<surface_vertices_struct_type>& surface_vertices, std::vector <surface_vertices_normal_struct_type> surface_vertices_normals, size_t number_vertices_to_export, voxel_object_data_class& voxel_object_data, bool export_colors) {
		...
		for (index = 0; index < number_vertices_to_export; index++) {
			...
			if (export_colors) {
				...
				stream << voxel_cartesian_coordinate.x << " " << voxel_cartesian_coordinate.y << " " << voxel_cartesian_coordinate.z << " ";
				stream << surface_vertices_normals[index].voxel_normal.x << " " << surface_vertices_normals[index].voxel_normal.y << " " << surface_vertices_normals[index].voxel_normal.z << std::endl;
			}
...
//																					 + "\n"+ std::string::number(vertex1_id) + " : " + std::string::number(vertex2_id) + " : " + std::string::number(vertex3_id));
				...
				stream << 3 << " " << vertex1_id << " " << vertex2_id << " " << vertex3_id << std::endl;
		}
	...
	void write_voxel_face_ply_vertex_data_to_file(std::vector<glm::vec3>& vertices, std::vector <glm::vec3> normals, size_t number_vertices_to_export) {
		...
		for (index = 0; index < number_vertices_to_export; index++) {
			...
			stream << normals[index].x << " " << normals[index].y << " " << normals[index].z << std::endl;
		}
	...
	void write_voxel_face_ply_face_data_to_file(std::vector<std::array<int, 4>> faces, size_t number_faces_to_export) {
		...
		for (index = 0; index < number_faces_to_export; index++) {
			stream << "4 " << faces[index][0] << " " << faces[index][1] << " " << faces[index][2] << " " << faces[index][3] << std::endl;
		}
	}
