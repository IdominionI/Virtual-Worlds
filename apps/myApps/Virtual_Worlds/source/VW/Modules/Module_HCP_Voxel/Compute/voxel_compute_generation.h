...
#include <VW_FrameWork/Compute/compute_shader.h>
#include <VW_FrameWork/Compute/import_compute_expression_code.h>
...
public:
	...
	voxel_object_data_class *cloud_data = NULL;// pointer to the hcp voxel class in the virtual worlds scene data model
	...
	bool generate_voxel_function() {
		...
		if (cloud_data == NULL) return false;

		voxel_generator_parameters = cloud_data->voxel_generator_parameters;
...
//printf("critical, voxel_generation : generate_voxel_function::Could not create initial voxel data matrix.\n");
		...
		// It seems that for the compute shader to function and work referencing the voxel data, need to create a pointer to the vector data 
		// as there is a problem using the vector data() function within the opengl glBufferData and glGetBufferSubData functions
		int size = int(cloud_data->voxel_matrix_data.size());
//for(int i = 0; i< size;i++){
//	printf("generate_voxel_function 0000 : %i \n", cloud->voxel_object_data.voxel_matrix_data[i]);
//}
		voxel_data_type* buffer = cloud_data->voxel_matrix_data.data();
...
//printf("Critical : %s\n", error_msg.c_str());
		...
		shader.set_f1(progHandle, cloud_data->voxel_size, "voxel_size"); // this works unsigned int shader_program_id, float v, const std::string& name
		shader.set_f1(progHandle, cloud_data->voxel_generator_parameters.generation_threshold, "threshold"); // this works
		shader.set_i1(progHandle, GLint(cloud_data->voxel_generator_parameters.min_surface_value), "min_surface_value"); // this works
		shader.set_i1(progHandle, GLint(cloud_data->voxel_generator_parameters.max_surface_value), "max_surface_value"); // this works
		shader.set_f1(progHandle, cloud_data->voxel_generator_parameters.e_time, "e_time"); // this works
		shader.set_i1(progHandle, GLint(cloud_data->voxel_generator_parameters.frame), "frame"); // this works

		shader.set_vec3(progHandle, cloud_data->matrix_origin, "origin"); // this works
		shader.set_i1(progHandle, cloud_data->matrix_dimension.x, "matrix_dimension_x"); // this works
		shader.set_i1(progHandle, cloud_data->matrix_dimension.y, "matrix_dimension_y"); // this works
		shader.set_i1(progHandle, cloud_data->matrix_dimension.z, "matrix_dimension_z"); // this works
	...
	bool create_voxel_matrix(voxel_hcp_object_class* cloud, voxel_generator_parameters_struct_type  voxel_generator_parameters) {
		cloud_data->voxel_matrix_data.clear();
		cloud_data->voxel_matrix_data.shrink_to_fit();
...
//QMessageBox::information(0, "Function Expression Success", "create_voxel_matrix 00: "+QString::number(data_set_x_size)+":"+QString::number(data_set_y_size)+":"+QString::number(data_set_z_size)+":", QMessageBox::Ok);
		...
		cloud_data->voxel_size = voxel_generator_parameters.resolution_step;

		cloud_data->matrix_dimension = { data_set_x_size,data_set_y_size,data_set_z_size };
		cloud_data->matrix_origin = origin;
		cloud_data->create_empty_volume_cubic(data_set_x_size, data_set_y_size, data_set_z_size);
//QMessageBox::information(0, "Function Expression Success", "create_voxel_matrix 01: "+QString::number(cloud->voxel_object_data.voxel_matrix_data.size())+":", QMessageBox::Ok);
		...
		if (cloud_data->voxel_matrix_data.size() > 0)
			return true;
		else
			return false;
	}
