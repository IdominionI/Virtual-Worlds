...
public:
	...
	bool export_hex_surface_center_points_ply(vw_scene_objects_category_class&hex_surface_entities_to_export,int export_selection, id_type entity_id, bool export_colors = false) {

		if (hex_surface_entities_to_export.category_objects.size() < 1) {
	...
	bool export_hex_surface_center_points_ply_selected(std::string directory_pathname, vw_scene_objects_category_class& hex_surface_entities_to_export, id_type entity_id, bool export_colors) {
		...
		vw_object_base_class *object_to_export = hex_surface_entities_to_export.get_entity_object(entity_id);
		hex_surface_object_class *hex_surface_object_to_export = dynamic_cast<hex_surface_object_class *>(object_to_export);
		...
		filename_to_write = directory_pathname + "/hcp_" + hex_surface_object_to_export->name + ".ply";
...
/*
	bool export_hex_surface_center_points_ply_active(std::string directory_pathname, hex_surface_scene_objects_class& hex_surface_entities_to_export, bool export_colors) {
		for (index_type i = 0; i < hex_surface_entities_to_export.category_objects.size(); i++) {
			...
			if (hex_surface_object_to_export->active_object) {
				if (hex_surface_object_to_export->hex_surface_object_data.hex_surface_matrix_data.size() > 0) {
					filename_to_write = directory_pathname + "/hcp_" + std::to_string(i) + "_" + hex_surface_object_to_export->name + ".ply";
...
*/
	...
	bool export_hex_surface_faces_ply(vw_scene_objects_category_class&hex_surface_entities_to_export,int export_selection, id_type entity_id) {
	...
	//bool export_hex_surface_faces_ply_selected(std::string directory_pathname,vw_scene_objects_category_class &hex_surface_entities_to_export, id_type entity_id) {
	bool export_hex_surface_faces_ply_selected(std::string directory_pathname, vw_scene_objects_category_class &hex_surface_entities_to_export, id_type entity_id) {
		...
		filename_to_write = directory_pathname + "/hs_" + hex_surface_object_to_export->name + ".ply";
...
/*
	bool export_hex_surface_faces_ply_active(std::string directory_pathname, hex_surface_scene_objects_class &hex_surface_entities_to_export) {
		for (index_type i = 0; i < hex_surface_entities_to_export.category_objects.size(); i++) {
			...
			if (hex_surface_object_to_export->active_object) {
				if (hex_surface_object_to_export->hex_surface_object_data.hex_surface_matrix_data.size() > 0) {
					filename_to_write = directory_pathname + "/hs_" + std::to_string(i) + "_" + hex_surface_object_to_export->name + ".ply";
...
private:
	...
	bool export_object_data_ply_format(hex_surface_object_data_class &hex_surface_object_data, bool export_colors) {
		...
		for (hex_index = 0; hex_index < number_hexes_to_export; hex_index++) {
			...
			stream << hex_surface_object_data.grid_origin.x + hex_cartesian_coordinate.x << " " << hex_surface_object_data.grid_origin.y + hex_cartesian_coordinate.y << " " << hex_surface_value << std::endl;
	...
	void create_file_ply_header(size_t number_hexs_to_export, bool export_colors) {
		...
		stream << "element vertex " << number_hexs_to_export << std::endl;
		...
		for (size_t j = 0; j < y_dim; j++) {
			...
			for (size_t i = 0; i < x_d; i++) {
				...
				if (j % 2 == 0 && j > 0) { // have even row
					if (i > 0) {
						stream << "3 " << n - x_dim << " " << n << " " << n - 1 << std::endl;
						if (i < x_dim - 1) {
							stream << "3 " << n - x_dim + 1 << " " << n << " " <<  n - x_dim << std::endl;
						}
					}
				}
