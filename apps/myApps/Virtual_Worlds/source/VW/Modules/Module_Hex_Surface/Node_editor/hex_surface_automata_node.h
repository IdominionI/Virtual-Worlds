...
#include "../Animation/hex_surface_automata_animation.h" // +++++++++++

class hex_surface_automata_node_class : public node_basis_class {
public:
    ...
    std::vector<hex_surface_automata_rule_struct_type> hex_automata_byte_rules;
   ...
   void export_node(std::fstream& stream) {
    ...
    bool import_node(std::vector<std::string> lines, int& line_number) {
        return import_hex_surface_automata_rules(lines, line_number);
    }
    ...
    bool import_hex_surface_automata_rules(std::vector<std::string> lines, int& line_number) {
        ...
        return import_export_hex_surface_automata_rules.read_automata_byte_rules_into_byte_rules_parameters(lines, hex_automata_byte_rules, line_number);
    }
...
private:
    ...
    void display_voxel_automata_widget(hex_surface_object_class* voxel_hcp_object_to_execute);
};
