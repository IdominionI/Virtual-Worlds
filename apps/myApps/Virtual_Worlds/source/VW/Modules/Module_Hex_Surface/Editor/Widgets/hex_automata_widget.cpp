...
#include <Universal_FW/Tools/dialogs.h>
	...
	void hex_surface_automata_widget_class::display() {
		if (hex_surface_automata_rules == NULL) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR:: Cannot define/execute an automata hex surface rule. No Hex surface rules defined.\n");
			return;
		}
		...
		hex_surface_automata_rules_widget.rules_manager(*hex_surface_automata_rules);
...
//printf("setup_play 00");
		...
		hex_surface_automata_functions.hex_surface_automata_rules    = *hex_surface_automata_rules;
...
//printf("INFO", "next_step_voxel_automata 00");
		...
		if (!hex_surface_object_to_execute) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR", "next_step_voxel_automata_no_thread():: No hex surface object specified to execute");
...
//printf("INFO", "perform_automata_step 00 : %i\n" , step);
		...
		if (!hex_surface_object_to_execute) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR", "perform_automata_step():: No hex surface object specified to execute");
			return false;
		}
...
//		}
		...
		//hex_surface_object_to_execute->set_voxel_value_range(MIN_VOXEL_VALUE, MAX_VOXEL_VALUE);// This is to be changed to a user defined input in a future update
...
*/
	...
	void hex_surface_automata_widget_class::automata_functions_logging(log_panel_class* log_panel) {
