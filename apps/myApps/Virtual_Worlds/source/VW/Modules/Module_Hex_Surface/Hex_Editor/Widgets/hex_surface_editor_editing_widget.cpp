...
void hex_surface_editor_editing_widget_class::save_gradient() {
...
//ImGG::Gradient grad_list = hcp_analysis_shader->gradient_widget.gradient();
	ImGG::Gradient grad_list = hex_analysis_shader->gradient_widget.gradient();
...
//std::cout << "hcp_voxel_editor_editing_widget_class:: update_editor_shader_gradient " << pos << ":" << i << std::endl;
	...
	if (hex_surface_editor_import_export.export_gradient(file_pathname,interpolation_mode_s,gradient_data)) {
		if (log_panel != NULL) log_panel->application_log.AddLog("INFO :Voxel volume generation parameter data saved to file\n %s\n", file_pathname);
	}

}

void hex_surface_editor_editing_widget_class::load_gradient() {
	...
	if (!hex_surface_editor_import_export.import_gradient(file_pathname,interpolation_mode_s,gradient_data)) {
		if (log_panel != NULL) log_panel->application_log.AddLog("Error : Gradient data could not be imported from file\n %s\n", file_pathname);
		return;
	}


	hex_analysis_shader->gradient_widget.gradient().clear();
	
	if (interpolation_mode_s == "Linear")
		hex_analysis_shader->gradient_widget.gradient().interpolation_mode() = ImGG::Interpolation::Linear;
	else
		hex_analysis_shader->gradient_widget.gradient().interpolation_mode() = ImGG::Interpolation::Constant;
	...
	for (std::pair<float,glm::vec4> gradient_mark : gradient_data) {
		...
		hex_analysis_shader->gradient_widget.gradient().add_mark(mark);
	}
