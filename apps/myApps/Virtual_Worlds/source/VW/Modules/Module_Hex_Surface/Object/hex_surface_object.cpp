...
bool hex_surface_object_class::define_geometry_data() {
	...
	//point_cloud.vertices.delete_geometry(true);// +++
	point_cloud->delete_geometry(true);// +++
...
bool hex_surface_object_class::define_vbo_point_normals() {
	...
	for (int j = 0; j < y_dim; j++) {
		...
		for (int i = 0; i < x_d; i++) {
			...
			if (j % 2 == 0 && j > 0) { // have even row
				if (i > 0) {
					vertex_0 = point_cloud->getVertex(n - x_dim);
					vertex_1 = point_cloud->getVertex(n);
					vertex_2 = point_cloud->getVertex(n - 1);
					//vertex_0 = point_cloud.vertices.vertices[n - x_dim];
					//vertex_1 = point_cloud.vertices.vertices[n];
					//vertex_2 = point_cloud.vertices.vertices[n - 1];

					glm::vec3 normal = calculate_normal_vector(vertex_0, vertex_1, vertex_2);

					point_cloud->setNormal(n - x_dim, normal);
					point_cloud->setNormal(n, normal);
					point_cloud->setNormal(n - 1, normal);
					//point_cloud.vertices.vertex_normals[n - x_dim] = normal;
					//point_cloud.vertices.vertex_normals[n] = normal;
					//point_cloud.vertices.vertex_normals[n - 1] = normal;

					//stream << "3 " << n - x_dim << " " << n << " " << n - 1 << endl;
					if (i < x_dim - 1) {

						vertex_0 = point_cloud->getVertex(n - x_dim + 1);
						vertex_1 = point_cloud->getVertex(n);
						vertex_2 = point_cloud->getVertex(n - x_dim);
						//vertex_0 = point_cloud.vertices.vertices[n - x_dim + 1];
						//vertex_1 = point_cloud.vertices.vertices[n];
						//vertex_2 = point_cloud.vertices.vertices[n - x_dim];

						glm::vec3 normal = calculate_normal_vector(vertex_0, vertex_1, vertex_2);

						point_cloud->setNormal(n - x_dim + 1,normal);
						point_cloud->setNormal(n, normal);
						point_cloud->setNormal(n - x_dim,normal);
						//point_cloud.vertices.vertex_normals[n - x_dim + 1] = normal;
						//point_cloud.vertices.vertex_normals[n] = normal;
						//point_cloud.vertices.vertex_normals[n - x_dim] = normal;
...
bool hex_surface_object_class::define_vbo_face_normals() {
	//size_t number_vertices = point_cloud.vertices.get_number_of_vertices();
	size_t number_vertices = point_cloud->getNumVertices();
	...
	for (int i = 0; i < number_triangles; i++) {
		int offset = i * 3;
		glm::vec3 vertex_0 = point_cloud->getVertex(offset);
		glm::vec3 vertex_1 = point_cloud->getVertex(offset + 1);
		glm::vec3 vertex_2 = point_cloud->getVertex(offset + 2);

		//glm::vec3 vertex_0 = point_cloud.vertices.vertices[offset];
		//glm::vec3 vertex_1 = point_cloud.vertices.vertices[offset + 1];
		//glm::vec3 vertex_2 = point_cloud.vertices.vertices[offset + 2];

		glm::vec3 normal = calculate_normal_vector(vertex_0, vertex_1, vertex_2);

		point_cloud->setNormal(offset, normal);
		point_cloud->setNormal(offset + 1,normal);
		point_cloud->setNormal(offset + 2,normal);
		//point_cloud.vertices.vertex_normals[offset] = normal;
		//point_cloud.vertices.vertex_normals[offset + 1] = normal;
		//point_cloud.vertices.vertex_normals[offset + 2] = normal;
	}
